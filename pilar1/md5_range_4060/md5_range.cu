...
// ---- Grid: tamaño de bloque fijo, cantidad de bloques calculada en runtime ----
// (ver main(): cudaOccupancyMaxActiveBlocksPerMultiprocessor + multiProcessorCount
// reemplaza un valor hardcodeado para una GPU específica por una cifra derivada
// del hardware real detectado al ejecutar el binario, sin recompilar)
#define THREADS_PER_BLOCK 256

// Padded-message buffer per thread (handles base strings up to ~240 chars).
// NOTA: este array se indexa dinámicamente, por lo que reside en memoria
// local por thread (no en registros), generando tráfico a memoria en cada
// hash calculado. Reducir este valor mejora rendimiento pero limita la
// longitud máxima del string base aceptado. Se mantiene en 320 para no
// alterar el comportamiento funcional del programa.
...
__global__ void bruteForceRange(
    ...
    int*         found_flag,         // 0 = searching, 1 = done
    ...
    for (uint64_t nonce = range_min + idx; nonce <= range_max; nonce += stride) {
        ...
        // Lectura volátil: no se necesita atomicidad para este chequeo de
        // salida temprana, solo evitar que el compilador cachee el valor
        // en un registro entre iteraciones. Reemplaza atomicAdd(flag, 0),
        // que generaba una operación atómica global real en cada iteración
        // de cada thread.
        if (*((volatile int*)found_flag) != 0) return;
...
int main(int argc, char** argv) {
    ...
    CUDA_CHECK(cudaMalloc(&d_base,        base_len   + 1));
    CUDA_CHECK(cudaMalloc(&d_target,      target_len + 1));
    CUDA_CHECK(cudaMalloc(&d_found_flag,  sizeof(int)));
    CUDA_CHECK(cudaMalloc(&d_found_nonce, sizeof(uint64_t)));
    CUDA_CHECK(cudaMalloc(&d_found_hash,  16));

    CUDA_CHECK(cudaMemcpy(d_base,   base,   base_len   + 1, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_target, target, target_len + 1, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemset(d_found_flag, 0, sizeof(int)));

    // ---- Calcular grid según GPU detectada en runtime ----
    cudaDeviceProp prop;
    CUDA_CHECK(cudaGetDeviceProperties(&prop, 0));
    int maxBlocksPerSM = 0;
    CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &maxBlocksPerSM, bruteForceRange, THREADS_PER_BLOCK, 0));
    int blocks = prop.multiProcessorCount * maxBlocksPerSM * 8; // 8 oleadas
    if (blocks <= 0) blocks = prop.multiProcessorCount; // fallback defensivo
    ...
    CUDA_CHECK(cudaMemcpy(&found_nonce, d_found_nonce, sizeof(uint64_t), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(found_hash,   d_found_hash,  16,               cudaMemcpyDeviceToHost));
    ...
    CUDA_CHECK(cudaFree(d_found_nonce));
    CUDA_CHECK(cudaFree(d_found_hash));
