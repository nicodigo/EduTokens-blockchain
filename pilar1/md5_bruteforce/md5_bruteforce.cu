...
__global__ void bruteForce(
    ...
    // Grid-stride loop: every thread walks its own arithmetic progression
    for (uint64_t nonce = idx; ; nonce += stride) {
...
int main(int argc, char** argv) {
    if (argc != 3) {
        fprintf(stderr, "Usage: %s <base_string> <target_prefix>\n", argv[0]);
        return 1;
    }
    ...
    // Worst case: base + 20 (nonce digits) + 1 (0x80) + 64 (zeros) + 8 (len)
    //            ≈ base + 93 bytes
    ...
    bruteForce<<<BLOCKS, THREADS_PER_BLOCK>>>(
        d_base, base_len,
        d_target, target_len,
    ...
    if (found_flag) {
        ...
        printf("\nNo solution found (uint64_t space exhausted).\n");
    }
